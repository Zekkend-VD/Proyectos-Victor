...
if "string" in form and "palabra" in form:
    frase = form.getvalue("string")
    palabra = form.getvalue("palabra")
    if frase and palabra:
        contiene = palabra in frase
        resultado = f"""
        <div class="result-card">
            <p><strong>Frase:</strong> {frase}</p>
            <p><strong>Palabra buscada:</strong> '{palabra}'</p>
            <p style="color: {'#2dd4bf' if contiene else '#f87171'}; font-weight:600;">
                {'✓ Contiene' if contiene else '✗ No contiene'} la palabra.
            </p>
        </div>
        """
    else:
...
<head>
    ...
    <title>Ejercicio 6 — Buscar palabra</title>
...
<body>
    <div class="container">
        <h1>🔍 Ejercicio 6 — Buscar palabra</h1>
        <form method="post">
            <label for="string">Frase:</label>
